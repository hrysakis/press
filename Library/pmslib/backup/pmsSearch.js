(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(['jquery'], function(jquery) {
      return (root.pms = factory(jquery));
    });
    ...
    // Browser globals
    root.pms = factory(root.jQuery);
  }
}(this, function($) {

  var PMSSearch = function(element, options, cb) {
    ...
    for (key in this.categoryDescendants){
      // console.log(this.categoryDescendants[key]);
    ...
    $.when(this.getCategories(), this.getTags()).always($.proxy(function(a1, a2) {
      if (!(a1) || !a2) {
        console.log('GET was unsuccesfull');
        return;
      }
  ...
  PMSSearch.prototype = {
    constructor: PMSSearch,
    setdbURL: function(url) { //Set dbURL
      this.dbURL = url;
    },
    getCategories: function() { //Get Categories and populate select fields
      return $.ajax({
    ...
    getTagField: function(response){
      ...
      console.log(tags);
      var required = '';
      $input = $('<input class="typeahead form-control input-sm pms-field" '+
    ...
    getAuthorField: function() {  //Typeahead Field for searching Authors
          ...
          this.bloodhounds[key] = new Bloodhound({
            ...
            remote: {
              ...
              prepare: (function(groupKey, prefix){
                ...
                '?uuid pms:Person_Group "'+groupKey+'". }';
      ...
      $input.bind('typeahead:select keypress', (function(that){
          ...
          $(ev.type !== 'keypress' && '.author-contributor-name').each(function() {
            // console.log($(this).attr('data-mail'));
    ...
    getOrgsField: function(organization) {  //Typeahead field for Organizations
    ...
    getCategoriesFields: function(category_tree) {  //Get Categories from DB and create <select>
    ...
    getPredefinedAuthors: function(authoruuids){
      ...
      query += '?uuid pms:Person_Group ?group. }';
    ...
    searchByFields: function(offset, filters, pushState, stateObj){
      ...
      if (authors.length === 0){
        ...
        authorQuery += '?con rdfs:subPropertyOf* pms:contributor. \n';
        for (var i=0; i<authors.length; i++){
          authorQuery += '?slot'+i+' pms:list_item <'+$(authors[i]).attr('data-uuid')+'>. \n';
          authorQuery += '?list'+i+' pms:slot ?slot'+i+'. \n';
      ...
      var orgQuery = '';
      if (orgs.length === 0){
        // console.log('None org Selected');
      }else if(orgs.length === 1){
        if (searchLabel !== '')
          searchLabel += ', ';

        orgQuery = '?pub pms:belongsTo pms:'+orgs.attr('id')+'. \n';
        ...
        orgQuery = '?pub pms:belongsTo ?org. \n'+
                   'FILTER(?org = pms:'+$(orgs[0]).attr('id');
        orgIds.push($(orgs[0]).attr('id'));
        for (var i=1; i<orgs.length; i++){
          orgQuery+= ' || ?org = pms:'+$(orgs[i]).attr('id');
      ...
      if (tags.length > 0){
        ...
        for (var i=0; i<tags.length; i++){
          tagQuery += '?pub pms:Tag "'+$(tags[i]).attr('id')+'". \n';
      ...
      // if (category !== ''){
      //   if(subcategory !== ''){
      //     catQuery += '?pub rdf:type pms:'+ subcategory + '. \n';
      //   }else{
      //     catQuery += '?pub rdf:type pms:'+ category + '. \n';
      //   }
      // }
      var reviewedQuery = '';
      if (category === '' && reviewed){
        ...
        reviewedQuery += 'FILTER (?ancClass = pms:Journal_Peer_reviewed || ?ancClass = pms:Conf_Peer_reviewed). \n';
        ...
        if (!reviewed){
          if(subcategory !== ''){
            reviewedQuery += '?pub rdf:type ?type. \n';
            reviewedQuery += '?type rdfs:subClassOf* pms:'+subcategory+'. \n';
            searchLabel += this.category_labels[subcategory];
          }else{
            reviewedQuery += '?pub rdf:type ?type. \n';
            reviewedQuery += '?type rdfs:subClassOf* pms:'+category+'. \n';
      ...
      if (field !== ''){
        ...
        fieldQuery += '?pub rdf:type [rdfs:subClassOf* pms:Publication]. \n';
        fieldQuery += 'BIND(concat(str(?score), str(?pub)) as ?scoreOrder). \n';
        fieldQuery += 'MINUS {?pub pms:English_Abstract ?searchField}. \n';
        fieldQuery += 'MINUS {?pub pms:Publication_URL ?searchField}. \n';
      }
      var restFields = '';
      if (reviewedQuery === ''){
      ...
      if (yearQuery === ''){
        restFields += '?pub pms:Year ?year. \n';
      }
      if (fieldQuery === 0){
        restFields += 'BIND(concat(str(?year), str(?pub)) as ?order). \n';
      }
      restFields += 'BIND(strafter(str(?type), "#") AS ?typeID). \n'+
                  // '?pub pms:belongsTo ?org. \n'+
                  // '?org pms:Organization_Name ?orgName. \n'+
                  'OPTIONAL{?pub pms:External_Link ?External_Link}. \n'+
                  'OPTIONAL{?pub pms:Book_Title ?Book_Title}. \n'+
                  'OPTIONAL{?pub pms:Chapter_Title ?Chapter_Title}. \n'+
                  'OPTIONAL{?pub pms:English_Title ?English_Title}. \n'+
                  'OPTIONAL{?pub pms:Local_Link ?Local_Link}. \n'+
                  'OPTIONAL{?pub pms:Publication_URL ?Publication_URL}. \n';
    ...
    searchByCategory: function(category_id, offset, filterValues, pushState){
      ...
      // document.title = 'Search "'+$element.text()+'"';
      // var stateObj = null; //TODO CONTINUE HERE - REMINDER DO NOT USE ?q
      //
      ...
      var allRequired = {};           //Get same required  fields for all categories & subcategories
      ...
      var selectorQuery = '?pub rdf:type [rdfs:subClassOf* pms:'+$element.attr('id')+']. \n';
      ...
      var fieldQuery = '?pub pms:Year ?year. \n'+
      ...
      for (var key in this.personFields) {
       if (this.personFields.hasOwnProperty(key)) {
         if (key in allRequired){
           ...
           authorQuery += '?pub pms:'+ key + ' ?' + key +'s. \n';
           authorQuery += '?'+key+'s pms:slot ?'+key+'slot. \n';
           authorQuery += '?'+key+'slot pms:list_item ?'+key+'uuid. \n';
           authorQuery += '?'+key+'slot pms:list_index ?'+key+'index. \n';
    ...
    getPublicationsContributors: function(publications, contributorTypes){
      var query = 'prefix pms: <'+this.prefix+'> \n';
      query += 'SELECT * WHERE { \n';
      ...
      for (var key in this.personFields) {
       if (this.personFields.hasOwnProperty(key)) {
         if (key in contributorTypes){
           ...
           query += '). \n';
           query += '?pub pms:Year ?year. \n';
           query += 'BIND(concat(str(?year), str(?pub)) as ?order). \n';
           query += 'BIND("'+key+'" as ?type). \n';
           query += '?'+key+'s pms:slot ?'+key+'slot. \n';
           query += '?'+key+'slot pms:list_item ?'+key+'uuid. \n';
           query += '?'+key+'slot pms:list_index ?'+key+'index. \n';
    ...
    getPublicationsMultipleFields: function(publications){
      ...
      // }
      // query += '). \n';
      // query += '?org pms:Organization_Name ?orgName. \n';
      // query += '}UNION{ \n';
      query += '?pub pms:Tag ?Tag. \n';
      query += 'FILTER (';
      for (var i=0; i<publications.length; i++){
        if (i>0) query += '||';
        query += '?pub = <'+ publications[i] + '> ';
    ...
    getFiltersValues($elements){
      ...
      $('.search-filter-contributor[data-selected="selected"]', $elements).each(function(index){
          ...
          contributorQuery += '?filterSlot'+index+' pms:list_item '+$(this).attr('data-oVal')+'. \n';
          contributorQuery += '?filterList'+index+' pms:slot ?filterSlot'+index+'. \n';
      ...
      if ($('.search-filter-contributor[data-selected="selected"]', $elements).length > 0)
        values.contributors.intro = '?filterCon rdfs:subPropertyOf* pms:contributor. \n';
    ...
    getFilters: function(prefixQuery, searchQuery){
      var completeQuery = prefixQuery +
                          ...
                          searchQuery+
                          '?pub ?p ?o. \n'+
                          'FILTER (?p = pms:Year || ?p = pms:belongsTo || ?p = rdf:type || ?p = pms:Tag)\n'+
    ...
    getAuthorFilters: function(prefixQuery, searchQuery){
      ...
      completeQuery += 'SELECT ?contributoruuid ?contributorgivenName '+
                          ...
                          searchQuery+
                          '?contributor rdfs:subPropertyOf* pms:contributor. \n'+
                          '?pub ?contributor ?contributors. \n'+
                          '?contributors pms:slot ?contributorslot. \n'+
                          '?contributorslot pms:list_item ?contributoruuid. \n'+
    ...
    getCount: function(prefixQuery, searchQuery){
      var completeQuery = prefixQuery +
                          ...
                          searchQuery+
                          '?pub rdf:type [rdfs:subClassOf* pms:Publication]. \n'+
                          '}} order by desc(?o)';
      return this.getQuery(completeQuery);
    },
    ...
    insertSearchResults: function(responsePublications, responseContributors, responseMultipleFields, requiredFields, searchLabel, offset, count){
      ...
      if (searchLabel === ''){
        console.log(this.lastSearchLabel);
      ...
      var contributors = {};
      console.log(responsePublications.results.bindings);
      console.log(responseContributors.results.bindings);
    ...
    insertFilters: function(fieldResponse, contributorResponse, selected){
      ...
      for(var i=0; i<tags.length; i++){
        var tagDiv = $('<div class="search-filter search-filter-tags" data-p="pms:Tag" '+
      ...
      for(var i=0; i<years.length; i++){
        var yearDiv = $('<div class="search-filter search-filter-year" data-p="pms:Year" '+
      ...
      for(var i=0; i<orgs.length; i++){
        var orgDiv = $('<div class="search-filter search-filter-org" data-p="pms:belongsTo" '+
  ...
  $.fn.pmsSearch = function(options, callback) {
    this.each(function() {
      var el = $(this);
      if (el.data('pmsSearch'))
        el.data('pmsSearch').remove();
      el.data('pmsSearch', new PMSSearch(el, options, callback));
    });
    return this;
  };
